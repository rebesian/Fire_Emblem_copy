...
void stageScene::update()
{
	if (action)
	{
		...
		{
			if (truePlayerFalseEnemy && phazeAlpha <= 0) // 플레이어 턴 && 플레이어턴 표시 끝날때 까지
			{
			...
			else if (!truePlayerFalseEnemy && phazeAlpha <= 0)// 에너미 턴 && 에너미턴 표시 끝날때 까지
			{
			...
			if (phazeAlpha >= 0) phazeAlpha -= 3; //표시 계속 내리기
		}
	}
...
void stageScene::render()
{
	...
	else
	{
		...
		{
			IMAGEMANAGER->findImage("플레이어페이즈")->alphaRender(_tileSet->getMapDC(), phazeAlpha);

		}
		else if (!truePlayerFalseEnemy && phazeAlpha >= 0)
		{
			IMAGEMANAGER->findImage("에너미페이즈")->alphaRender(_tileSet->getMapDC(), phazeAlpha);
		}
	}
